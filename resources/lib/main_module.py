...
class MainModule:
    ...
    def playtrailer(self):
        ...
        # fixed: traileractionbusy property IsEmpty -> String.IsEmpty, youtube listing file handling get first trailer
        # that is NOT dir and breaks loop trailer path input
    ...
    def playtraileryoutube(self):
        ...
        if not getCondVisibility("!String.IsEmpty(Window(Home).Property(traileractionbusy))"):
            ...
            xbmc.executebuiltin("ActivateWindow(busydialog)")
            title = self.params.get("title", "")
            trailer_mode = self.params.get("mode", "")
